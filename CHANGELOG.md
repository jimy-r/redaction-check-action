...
- The allow file is read from the base commit in `added-lines` mode, for the net diff and every commit in the range alike, so an entry a pull request adds lets nothing through until it has merged. `all-files` mode reads it as `HEAD` has it, and a diff on stdin reads it from disk as given. The list never applies to a file with the allow file's name, so a secret pasted into the allow file is still reported.
