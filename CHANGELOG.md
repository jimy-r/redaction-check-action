...
- `all-files` mode reads the allow file as `HEAD` commits it, and scans that committed copy as the allow file itself, whatever its size. A diff on stdin reads the allow file from disk as given. The list never applies to a file with the allow file's name, so a secret pasted into the allow file is still reported.
