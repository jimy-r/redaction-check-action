...
- **Private and link-local IPs, and `.local` hostnames** - the RFC 1918 ranges, RFC 3927 link-local, and mDNS-style `name.local` hosts, any of which can fingerprint a specific home or office network. A `.local.` segment inside a filename, as in `settings.local.json`, is not read as a host.
